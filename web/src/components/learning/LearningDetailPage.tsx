...
import { useLearningByIdQuery } from "@/hooks/queries/useLearningQuery";
import { useRelatedToolsQuery } from "@/hooks/queries/useRelatedToolsQuery";


export default function NewsDetailPage() {
  const params = useParams();
  const learningId = parseInt(params.id as string);
  const { data: learning, isLoading, error } = useLearningByIdQuery(learningId);
  const {data : relatedData} = useRelatedToolsQuery("Learning", learningId)
  
  const cards =  relatedData?.learnings.map((item) => (
      {
      id: item.id,
      image: item.image,
      title: item.title,
        ...
        <div className="bg-white rounded-[25px] border border-[#F2F2F2] overflow-hidden">
          ...
          <div className="relative">
            <div className="px-8 pt-6 pb-0">
              ...
              {/* Title */}
              <h1 className="md:mx-auto font-[inter] font-medium text-[20px] md:text-[25px] text-[#000000] mt-[20px]  mb-4 w-full sm:w-full md:w-3/4 pl-0 md:pl-4 ">
                {learning?.title}
              </h1>
              ...
              <div className=" mx-auto  w-full md:w-3/4 text-center items-center justify-start flex flex-wrap gap-2 mb-4 pl-0 md:pl-4">
                {learning?.learningTags && learning?.learningTags.length > 0 ? (
                  learning?.learningTags.map((tag, index) => (
                    <span
                      key={index}
                      className="px-[10px] py-[4px] border border-[#F2F2F2] text-[#5856D6] font-Nunito font-semibold text-[15px] rounded-[9px]"
                    >
                      {tag.tag?.name}
                    </span>
                  ))
                ) : (
                  <span className="px-[10px] py-[4px] border border-[#F2F2F2] text-[#5856D6] font-Nunito font-semibold text-[15px] rounded-[9px]">
                    No tags
                  </span>
                )}
              </div>
              ...
              <section className="  w-full md:w-3/4 mx-auto  pl-0 md:pl-4 flex gap-4">
                ...
                {learning?.published_date
                  ? new Date(learning?.published_date).toLocaleDateString()
                  : "Not published"}
                </p>
          ...
          <div className="p-8 space-y-6 ">
            <div className="w-full md:w-3/4 mx-auto pl-0 md:pl-4">
              ...
              <section>
                ...
                <div className="flex gap-2">
                  {learning?.learningCategories && learning?.learningCategories.length > 0 ? (
                    learning?.learningCategories.map((tag, index) => (
                      <span
                        key={index}
            ...
            <RelatedContent cards={cards || []} name={"learning"}/>
          </div>
        </div>
      </div>
    </div>
