...
import { useArticleByIdQuery } from "@/hooks/queries/useArticlesQuery";
import { ArtTags } from "@/services/articlesService";
...
export default function NewsDetailPage() {
  ...
  const { data: article, isLoading, error } = useArticleByIdQuery(articleId);
  const {data : relatedData} = useRelatedToolsQuery("Article", articleId)
  
  const cards =  relatedData?.articles.map((item) => (
      {
      id: item.id,
      image: item.image,
      title: item.headline,
      ...
      <BreadCrumb
        ...
        <div className="bg-white rounded-[25px] border border-[#F2F2F2] overflow-hidden">
            ...
            <div className="px-8 pt-6 pb-0">
              ...
              <div className=" mx-auto  w-full md:w-3/4 text-center items-center justify-start flex flex-wrap gap-2 mb-4 pl-0 md:pl-4">
                {article?.articleTags && article?.articleTags.length > 0 ? (
                  article?.articleTags.map((tag : ArtTags, index) => (
                    <span
                      key={index}
                      className="px-[10px] py-[4px] border border-[#F2F2F2] text-[#00C7BE] font-Nunito font-semibold text-[15px] rounded-[9px]"
                    >
                      {tag.tag?.name}
                    </span>
                  ))
                ) : (
                  <span className="px-[10px] py-[4px] border border-[#F2F2F2] text-[#00C7BE] font-Nunito font-semibold text-[15px] rounded-[9px]">
                    No tags
                  </span>
                )}
              </div>
              ...
              <section className="  w-full md:w-3/4 mx-auto  pl-0 md:pl-4 flex gap-4">
                <p className="font-[inter] font-medium text-[15px] text-[#808080]">
                  {article?.published_date || "Recently"}
                </p>
          ...
          <div className="p-8 space-y-6 ">
              ...
              <section>
                ...
                <div className="flex gap-2">
                  {article?.articleCategories && article?.articleCategories.length > 0 ? (
                    article?.articleCategories.map((tag , index) => (
                      <span
                        key={index}
            ...
            <RelatedContent cards={cards || []} name={"articles"}/>
          </div>
        </div>
      </div>
    </div>
