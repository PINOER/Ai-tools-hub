...
const ToolDetails = ({ tool, selectedScreenshots, is_featured, onFeaturedChange }: ToolDetailProps) => {
    ...
    <div className='flex flex-col gap-3  px-6'>
      <div className='flex flex-col gap-5 justify-center'>
        ...
        <div className='flex flex-col gap-2'>
          <p className='text-[#00000033] font-medium font-[inter] text-[15px] '>ABOUT</p>
          <p className=' text-[15px] text-[#000000] font-medium leading-[20px]'>
            {tool?.full_description}
          </p>
        </div>

        <div className='flex flex-col gap-0'>
          <p className='text-[#00000033] font-medium font-[inter] text-[15px] mb-1 uppercase'>KEY FEATURES</p>

          <ul className='list-disc mt-2 text-[15px] font-[inter] text-[#000000] font-medium leading-[20px] pl-5 space-y-1'>
            {tool?.features?.map((feature) => (
              <li className='mb-0' key={feature}>{feature}</li>
            ))}
          </ul>
        </div>

        <div className='flex flex-col gap-0'>
          <p className='font-medium font-[inter] text-[15px] mb-1 text-[#00000033] uppercase'>USE CASES</p>
          <ul className='list-disc mt-2 text-[15px] font-[inter] text-[#000000] font-medium leading-[20px] pl-5 space-y-1'>
            {tool?.use_cases?.map((useCase) => (
              <li className='mb-0' key={useCase}>{useCase}</li>
            ))}
          </ul>
        </div>

        <div className='flex flex-col gap-0'>
          <p className='font-medium font-[inter] text-[15px] mb-1 text-[#00000033] uppercase'>TAGS</p>
          <div className='flex flex-wrap gap-2 mt-1'>
            {tool?.tool_tags?.map(({ tag }, index) => (
              <p key={`${tag.id}-${index}`} className='px-2 font-Nunito font-semibold py-0.5 text-[12px] border border-[#F2F2F2] rounded-md text-xs text-gray-700'>
                #{tag.name}
              </p>
            ))}
          </div>
        ...
        <div className='flex flex-col gap-0'>
          <p className='font-medium font-[inter]  text-[15px] mb-1 text-[#00000033] uppercase'>PRICING</p>
        ...
        <div className='flex flex-col gap-0'>
          <p className='font-medium font-[inter]  text-[15px] mb-1 text-[#00000033] uppercase'>PLATFORM</p>
          <p className='flex flex-wrap gap-2 mt-1'>
            {Array.isArray(tool?.platform_availability)
              ? tool?.platform_availability?.map((platform) => (
                <span key={platform} className='px-2 font-Nunito font-semibold py-0.5 text-[12px] border border-[#F2F2F2] rounded-md text-xs text-gray-700'>
                  {platform}
                </span>
              ))
              : tool?.platform_availability
                ? <span className='border border-[#F2F2F2] px-2 py-1 rounded-md'>{tool.platform_availability}</span>
                : null}
          </p>
        </div>

        <div className='flex flex-col gap-0'>
          <p className='font-medium font-[inter] text-[15px] mb-1 text-[#00000033] uppercase'>SCREENSHOTS</p>
          <div className='flex gap-1 overflow-x-auto'>
            {/* Show selected screenshots (from form) */}
            {selectedScreenshots && selectedScreenshots.length > 0 &&
              selectedScreenshots.map((file: File | string, index: number) => {
                ...
                return (
                  <Image
                    ...
                    width={0}
                    height={0}
                    className='rounded-lg border object-cover w-[165px] h-[104px] md:w-[410px] md:h-[270px]'
                    unoptimized={true}
                  />
                );
              })}
            
            {/* Show tool screenshots (from tool object) */}
            {(!selectedScreenshots || selectedScreenshots.length === 0) && tool?.screenshots && tool.screenshots.length > 0 &&
