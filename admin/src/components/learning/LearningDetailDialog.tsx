...
import { LEARNING_DETAILS_TABS } from '@/lib/contants';
import DialogContainer from '@/components/DialogContainer';
import type { Learning } from '@/types/learning';
import LearningDetails from '@/components/learning/LearningDetail';
...
export const LearningDetailsDialog = ({
  learning,
  open,
  onOpenChange,
  onDelete,
  ...
  const [selectedTab, setSelectedTab] = useState(
    LEARNING_DETAILS_TABS.learning_details
  );
    ...
    <div>
      ...
      >
          ...
          <div className='flex items-center gap-4 text-sm'>
            <p
              className={`rounded-md py-1 px-4 cursor-pointer ${
                selectedTab === LEARNING_DETAILS_TABS.learning_details
                  ? 'bg-[#4D4D4D] text-white'
                  : 'bg-white border-2 border-[#F2F2F2]'
              }`}
              onClick={() =>
                setSelectedTab(LEARNING_DETAILS_TABS.learning_details)
              }
            >
              Learning details
            </p>
            <p
              className={`rounded-md py-1 px-4 cursor-pointer ${
                selectedTab === LEARNING_DETAILS_TABS.learning_analytics
                  ? 'bg-[#4D4D4D] text-white'
                  : 'bg-white border-2 border-[#F2F2F2]'
              }`}
              onClick={() =>
                setSelectedTab(LEARNING_DETAILS_TABS.learning_analytics)
              }
            >
              Learning analytics
            </p>
        ...
        <div className='w-[80%]! mx-auto'>
          ...
          {selectedTab === LEARNING_DETAILS_TABS.learning_details && (
            <LearningDetails learning={learning} />
          )}
        </div>
        {selectedTab === LEARNING_DETAILS_TABS.learning_analytics && (
