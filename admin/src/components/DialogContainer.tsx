...
const DialogContainer = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={`md:max-w-${maxWidth} max-h-[90vh] overflow-y-auto p-0 [&>button]:hidden`}
      >
        <DialogHeader>
          {title !== null ? (
            <DialogTitle className='flex px-6 py-4 justify-between items-center text-[27px] font-medium text-[#00000033] font-[inter]'>
              ...
              <svg
                ...
                className='cursor-pointer transition-opacity duration-200 hover:[&_path]:fill-opacity-100'
                onClick={() => {
                  onOpenChange(false);
                  // @ts-ignore
