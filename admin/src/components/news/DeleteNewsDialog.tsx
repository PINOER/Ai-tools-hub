...
function timeAgo(publishedTime: string) {
  const now = new Date();
    ...
    >
      <div className='flex flex-col gap-8 px-6'>
        ...
        <div className='flex flex-col gap-2'>
          <p className='text-black'>You are about to remove news?</p>
          <p className='text-[#00000080]'>
            This will permanently delete this news and all related data.
          </p>
        </div>
        ...
        <Card className=' gap-4'>
          ...
          <div className='flex flex-col'>
            ...
            <p>
              {news?.published_time 
                ? timeAgo(news.published_time) 
                : "No time set"}
            </p>              
            <div className='flex items-center gap-1'>
                {news.newsTags?.map((newsTag) => (
                  <p 
                    key={newsTag.id} 
                    className='text-[#007AFF] bg-white px-3 py-1 rounded-md'
                  >
                    {newsTag.tag?.name || ''}
                  </p>
                ))}
