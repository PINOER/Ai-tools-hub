...
type CreateNewsType = CreateNews;
...
export const CreateNewsDialog = ({
  ...
  const [finalNewsDetails, setFinalNewsDetails] = useState<any>(); // Added type
  ...
  const [time, setTime] = useState<string>('10:00'); // Added type
  ...
  const [selectPrimaryCategory, setSelectPrimaryCategory] = useState<{
    label: string;
    value: number;
  } | undefined>(); // Allow undefined initially
  const [selectSecondaryCategory, setSelectSecondaryCategory] = useState<{
    label: string;
    value: number;
  } | undefined>(); // Allow undefined initially

  const [selectedTags, setSelectedTags] = useState<any[]>([]); // Added type
  ...
  const allCategoriesAsOptions = categories.map((category) => {
    return {
      value: category.id,
      label: category.name,
    };
  ...
  const incomingTags = tags.map((tag) => {
    return {
      value: tag.id,
      label: tag.name,
    };
  ...
  const handleBackClick = () => {
    ...
    onOpenChange(false);
    resetAll(); // Call resetAll instead of just reset() for full reset
  };
  ...
  const isFormValidated = (currentTab: string): boolean => {
    if (currentTab === CREATE_NEWS_DIALOG_TABS.basic_info) {
      ...
      // You might also want to check headline and url_slug here from react-hook-form
      // However, handleSubmit will do this automatically if called with onSubmit
      return true;
    }

    if (currentTab === CREATE_NEWS_DIALOG_TABS.content) {
      // React-hook-form's handleSubmit will handle articleContent validation
      return true;
    }
  ...
  const nextButtonClick = async (formData: any) => {
    ...
    const news: CreateNews = { // Type assertion for clarity
      headline,
      seo_title,
      url_slug,
      ...
      visibility: selectedNewsVisibility === 'Featured on Homepage' ? 'Public' : 'Public', // Adjust if you have other visibility options
      allow_comments,
    ...
    const newsForDisplay = {
      ...news,
      // Add fields needed for preview components that aren't directly from CreateNews type
      categories: [
        selectPrimaryCategory?.label, // Use optional chaining
        selectSecondaryCategory?.label, // Use optional chaining
  ...
  const createNews = async (news: CreateNewsType) => {
    ...
    setLoading(true);
    try {
      const createdNews: News = await createNewsApi(news);
      setNews((prev: any) => [...prev, createdNews]);
      setCreatedNews(createdNews);
      onOpenChange(false);
      openConfirmationDialog(true);
      resetAll(); // Reset form after successful creation
    } catch (error) {
      console.error('Error creating news:', error);
      setFormGeneralError('Failed to create news. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCategoryChange = (
    ...
    if (type === 'primary') {
      setSelectPrimaryCategory(newValue || undefined); // Set to undefined if null
    } else {
      setSelectSecondaryCategory(newValue || undefined); // Set to undefined if null
    }
    // IMPORTANT: No filtering of incomingCategories here.
    // The options for the dropdowns will be derived dynamically.
  };

  const handleFeaturedImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) {
      setFeaturedImage(null); // Clear image if no file selected
      return;
    }
  ...
  const resetAll = () => {
    ...
    setSelectPrimaryCategory(undefined); // Reset categories
    setSelectSecondaryCategory(undefined); // Reset categories
    setSelectedTags([]);
    setFormGeneralError('');
  ...
  return (
    ...
    >
      <Tabs
        options={createNewsDialogOptions}
          ...
          <div className='flex flex-col gap-5 justify-center px-6 py-4'>
            <div className='flex flex-col gap-1'>
              ...
              <Input
                placeholder='Type'
                {...register('headline', {
                  required: true, // Always required for basic info tab
                })}
              />
              {errors.headline && (
            ...
            <div className='flex gap-4'>
              <div className='flex flex-col gap-1 w-3/6'>
                ...
                <ReactSelect
                  ...
                  value={selectPrimaryCategory} // Ensure selected value is displayed
                  onChange={(newValue) =>
              ...
              <div className='flex flex-col gap-1 w-3/6'>
                ...
                <ReactSelect
                  options={availableCategoriesForSelect}
                  value={selectSecondaryCategory} // Ensure selected value is displayed
                  onChange={(newValue) =>
            ...
            <div className='flex flex-col gap-1'>
              ...
              <ReactSelect
                ...
                isMulti
                onChange={(newValues) => {
                  // Ensure correct typing for newValues if needed
                  setSelectedTags(newValues as any[]);
                }}
                value={selectedTags} // Ensure selected tags are displayed
              />
            </div>
          ...
          <div className='flex flex-col gap-5 justify-center px-6'>
            <div className='flex flex-col gap-1'>
              ...
              <Controller
                ...
                control={control}
                rules={{
                  required: true, // Always required for content tab
                }}
                render={({ field }) => (
