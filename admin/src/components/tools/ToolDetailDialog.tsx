...
import { TOOL_DETAILS_TABS } from '@/lib/contants';
...
export const ToolDetailsDialog = ({
  tool,
  open,
  onOpenChange,
  onDelete,
  ...
  const [selectedTab, setSelectedTab] = useState(
    TOOL_DETAILS_TABS.tool_details
  );
  ...
  return (
    <div>
      ...
      >
          ...
          <div className='flex items-center gap-4 text-sm'>
            <p
              className={`rounded-md py-1 px-4 cursor-pointer ${
                selectedTab === TOOL_DETAILS_TABS.tool_details
                  ? 'bg-[#4D4D4D] text-white'
                  : 'bg-white border-2 border-[#F2F2F2]'
              }`}
              onClick={() => setSelectedTab(TOOL_DETAILS_TABS.tool_details)}
            >
              Tool details
            </p>
            <p
              className={`rounded-md py-1 px-4 cursor-pointer ${
                selectedTab === TOOL_DETAILS_TABS.tool_analytics
                  ? 'bg-[#4D4D4D] text-white'
                  : 'bg-white border-2 border-[#F2F2F2]'
              }`}
              onClick={() => setSelectedTab(TOOL_DETAILS_TABS.tool_analytics)}
            >
              Tool analytics
            </p>
          </div>
        ...
        <ToolDetailGeneric tool={tool} />
        {selectedTab === TOOL_DETAILS_TABS.tool_details && (
          <ToolDetails tool={tool} 
          selectedScreenshots={tool.screenshots}
