    ...
    >
      <div className='flex flex-col gap-8 p-4'>
        ...
        <div className='flex flex-col gap-2'>
          <p className='text-black font-medium text-[20px] font-[inter]'>You are about to remove tool?</p>
          <p className='text-[#00000080] text-[20px] font-medium font-[inter]'>
            This will permanently delete this tool and all related data.
          </p>
        </div>
        ...
        <Card className='flex-row gap-4'>
          ...
          <div className='flex flex-col'>
            <p className='font-[inter] font-medium text-[15px]'>{tool.name}</p>
            <p className='text-[#808080] text-[15px] font-[inter] font-medium'>{tool.full_description}</p>
            <div className='flex items-center gap-2'>
              {tool?.tool_tags?.map(({ tag }: { tag: any }) => (
                <p className='text-[#007AFF] bg-white px-3 py-1 rounded-md font-Nunito font-semibold text-[12px]' key={tag.name}>
                  {tag.name}
                </p>
              ))}
            </div>
        ...
        <div className='flex flex-col gap-2'>
          <p className='text-[#808080] text-[14px] font-[inter] font-medium'>Reason for Deletion (optional)</p>

          <div className='flex flex-col gap-2 w-fit'>
            {REASONS_FOR_DELETION.map((reason) => (
              <p
                key={reason}
                className={`${
                  selectedReasons.includes(reason)
                    ? 'bg-[#4D4D4D] text-white font-Nunito font-semibold text-[15px]'
                    : 'bg-white text-black font-Nunito font-semibold text-[15px]'
