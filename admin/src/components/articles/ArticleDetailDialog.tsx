...
import { ARTICLE_DETAILS_TABS } from '@/lib/contants';
...
export const ArticleDetailsDialog = ({
  article,
  open,
  onOpenChange,
  onDelete,
  ...
  const [selectedTab, setSelectedTab] = useState(
    ARTICLE_DETAILS_TABS.article_details
  );
          ...
          <div className='flex items-center gap-4 text-sm'>
            <p
              className={`rounded-md py-1 px-4 cursor-pointer ${
                selectedTab === ARTICLE_DETAILS_TABS.article_details
                  ? 'bg-[#4D4D4D] text-white'
                  : 'bg-white border-2 border-[#F2F2F2]'
              }`}
              onClick={() =>
                setSelectedTab(ARTICLE_DETAILS_TABS.article_details)
              }
            >
              Article details
            </p>
            <p
              className={`rounded-md py-1 px-4 cursor-pointer ${
                selectedTab === ARTICLE_DETAILS_TABS.article_analytics
                  ? 'bg-[#4D4D4D] text-white'
                  : 'bg-white border-2 border-[#F2F2F2]'
              }`}
              onClick={() =>
                setSelectedTab(ARTICLE_DETAILS_TABS.article_analytics)
              }
            >
              Article analytics
            </p>
