...
import { useDeleteArticleMutation } from '@/hooks/queries/useArticlesQueries';
    ...
    >
      <div className='flex flex-col gap-8 p-4'>
        ...
        <div className='flex flex-col gap-2'>
          <p className='text-black'>You are about to remove article?</p>
          <p className='text-[#00000080]'>
            This will permanently delete this article and all related data.
          </p>
        </div>
        ...
        <Card className='flex-row gap-4'>
          ...
          <div className='flex flex-col'>
            <p className='font-[inter] font-medium text-[15px]'>{article.headline}</p>
            <div className='flex items-center gap-2'>
              {article?.articleTags?.map((tag: string, index: number) => (
                <p className='text-[#007AFF] bg-white px-3 py-1 rounded-md font-Nunito font-semibold text-[12px]' key={`${tag}-${index}`}>
                  #{tag}
                </p>
              ))}
            </div>
        ...
        <div className='flex items-center gap-3'>
          ...
          <button
            ...
            disabled={deleteArticleMutation.isPending}
          >
            {deleteArticleMutation.isPending && <LoadingSpinner />}
            {deleteArticleMutation.isPending ? 'Deleting...' : 'Confirm'}
          </button>
        </div>
      </div>
    </DialogContainer>
